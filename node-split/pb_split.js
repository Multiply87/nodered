module.exports = function(RED) {
    function Pb_SplitNode(config) {
        ...
        node.on('input', function (msg) {
            ...
            let FIRST_TASK = (config.typefirstTask === 'num' ? config.firstTask : msg[config.firstTask]) || 0;  //Задержка первой таски
            let SPLIT_DELAY = (config.typedelay === 'num' ? config.delay : msg[config.delay]) || 1000; //Интервал отправки таски
            const SPLIT_DELIMITER = config.splitBy.replace(/\\t/g, "\t")
                                                  .replace(/\\n/g, "\n")
                                                  .replace(/\\r/g, "\r") || "\n"; //разбить строку по указанному символу

            FIRST_TASK = FIRST_TASK > 600000 ? 600000 : FIRST_TASK;  //Ограничение не более 10 минут
            SPLIT_DELAY = SPLIT_DELAY > 600000 ? 600000 : SPLIT_DELAY; //Ограничение не более 10 минут
